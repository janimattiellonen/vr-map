...
task 'dev', 'start dev env', ->
    ...
    # also compile any coffeescript code found in web/src into web/js
    # so that we can also use coffeescript on the client side
    options = ['-c', '-b', '-w', '-o', 'web/js', 'web/src']
    cmd = which.sync 'coffee'
    coffee = spawn cmd, options
    coffee.stdout.pipe process.stdout
    coffee.stderr.pipe process.stderr
    ...
    # watch_js
    supervisor = spawn 'node', ['./node_modules/supervisor/lib/cli-wrapper.js','-w','app,views,web/src', '-e', 'coffee|js|jade', 'server']
